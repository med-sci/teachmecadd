...
class DataLoader:
    ...
    def _get_num_records(self) -> None:
        url: str = self.ACTIVITY_URL+f'?limit=5&offset=0&target_chembl_id={self.target}'
    ...
    def _create_pagination(self)-> None:
        ...
        if self.num_records <= self.MAX_RECORDS_ON_PAGE:
            ...
            self.num_full_pages: int = (self.num_records // self.MAX_RECORDS_ON_PAGE)+self.LAST_PAGE_INDEX
    ...
    def load_data(self)-> None:
        ...
        if self.num_full_pages != self.LAST_PAGE_INDEX:
            ...
            for _ in range(self.num_full_pages):
                url: str = self.ACTIVITY_URL+f'?limit={self.MAX_RECORDS_ON_PAGE}&offset={offset}&target_chembl_id={self.target}'
                response: Response = requests.get(url)
                ...
                if response.status_code == self.OK_RESPONSE_STATUS_CODE:
                    ...
                    raise HTTPError (f"Get response code: {response.status_code}")
        
        url: str = self.ACTIVITY_URL+f'?limit={self.last_page_count}&offset={offset}&target_chembl_id={self.target}'
        response: Response = requests.get(url)
